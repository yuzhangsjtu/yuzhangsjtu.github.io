...
|      | Expression                 | Function                                                     |
| ---- | -------------------------- | ------------------------------------------------------------ |
| 1    | A(1)                       | Reshapes the 2-D matrix **A** into a 1-D array and returns the first element of the array |
...
| 4    | A(: , j : k)               | Returns the submatrix formed by the column vectors from the j-th column to the k-th column of the 2-D matrix **A** |
| 5    | A( i : k , :)              | Returns the submatrix formed by the row vectors from the i-th row to the k-th row of the 2-D matrix **A** |
| 6    | A( i : k , j : m)          | Returns the submatrix formed by the intersection of the row vectors from the i-th row to the k-th row and the column vectors from the j-th column to the m-th column of the 2-D matrix **A** |
| 7    | A(:)                       | Merges every column of the 2-D matrix **A** into a single column vector |
| 8    | A( j : k)                  | Returns a row vector whose elements are the j-th element through the k-th element of A(:) |
...
| 12   | A([ i1 i2 …] , [ j1 j2 …]) | Returns the elements at the j1-th column, j2-th column, etc. and the i1-th row, i2-th row, etc. of matrix **A** |
...
| 2    | ones(m,n,……,p)  | Creates an m×n×……×p matrix of 1s                    |
| 3    | ones(size(A))   | Creates a matrix of 1s with the same size as matrix A |
| 4    | zeros(n)        | Creates an n×n matrix of 0s                         |
| 5    | zeros(m,n,……,p) | Creates an m×n×……×p matrix of 0s                    |
| 6    | zeros(size(A))  | Creates a matrix of 0s with the same size as matrix A |
